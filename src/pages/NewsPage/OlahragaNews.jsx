...
import { useFetchOlahragatNews } from "../../hooks/useFetchOlahragaNews"
const OlahragaNews = () => {
    // get data from hooks
    const { olahragaNews, error, loading } = useFetchOlahragatNews()
    if (loading) {
        return (
    ...
    let currentPost = olahragaNews.slice(firstPostIndex, lastPostIndex)

    return (
        <>
            <Header />
            <main className="h-full relative top-32 p-7 lg:p-20">
                <TittleSection name={"Olahraga"} />
                <article className='mt-10'>
                    <div className='lg:grid grid-cols-4 gap-16'>
                        {currentPost.map((item =>
                            <CardNews key={item.link} news={item} category={"olahraga"} />
                        ))}
                    </div>
                </article>
            </main>
