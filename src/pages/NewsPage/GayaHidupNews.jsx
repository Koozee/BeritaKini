...
import { useGayaHidupNews } from "../../hooks/useFetchGayaHidupNews"
const GayaHidupNews = () => {
    // get data from hooks
    const { gayaHidupNews, error, loading } = useGayaHidupNews()
    if (loading) {
        return (
    ...
    let currentPost = gayaHidupNews.slice(firstPostIndex, lastPostIndex)

    return (
        <>
            <Header />
            <main className="h-full relative top-32 p-7 lg:p-20">
                <TittleSection name={"Gaya Hidup"} />
                <article className='mt-10'>
                    <div className='lg:grid grid-cols-4 gap-16'>
                        {currentPost.map((item =>
                            <CardNews key={item.link} news={item} category={"gaya hidup"} />
                        ))}
                    </div>
                </article>
            </main>
