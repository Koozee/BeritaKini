...
import { useFetchNasionalNews } from "../../hooks/useFetchNasionalNews"
const NasionalNews = () => {
    // get data from hooks
    const { nasionalNews, error, loading } = useFetchNasionalNews()
    if (loading) {
        return (
    ...
    let currentPost = nasionalNews.slice(firstPostIndex, lastPostIndex)

    return (
        <>
            <Header />
            <main className="h-full relative top-32 p-7 lg:p-20">
                <TittleSection name={"Nasional"} />
